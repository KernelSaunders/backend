...
#put the constants into variables cleaner than hardcoding dict lookups
...
def distribute_percentages(n: int) -> list[int]:
    ...
    nums = [random.randint(1, 99) for i in range(n)]
    tot = sum(nums)
    weighted = [int(n/tot*100) for n in nums]

    #rounding due to integer conversion
    weighted[random.randint(0, n-1)] += 100 - sum(weighted)
    return weighted
...
def generate_claims(products: list[dict]) -> list[dict]:
    ...
    for p in products:
        for claim_type in random.sample(CLAIM_TYPES, k=random.randint(1, 3)):
            ...
            clean_claim = claim_type.replace('_', ' ')
            claims.append({
                "claim_id": gen_id("clm"),
                "product_id": p["product_id"],
                "claim_type": claim_type,
                "claim_text": f"This product is {clean_claim}.",
...
def generate_issues(products: list[dict], count: int) -> list[dict]:
    ...
    for i in range(count):
        ...
        resolution_note = pick(DESCRIPTIONS["resolution_notes"]) if status == "resolved" else None
...
def generate_change_logs(products: list[dict], stages: list[dict], claims: list[dict]) -> list[dict]:
    things = {
        "product": [i["product_id"] for i in products],
        "stage": [i["stage_id"] for i in stages],
        "claim": [i["claim_id"] for i in claims],
    }
    logs = []
    for entity_type, ids in things.items():
        for entity_id in random.sample(ids, min(10, len(ids))):
            logs.append({
                ...
                "entity_id": entity_id,
                "changed_by": gen_id("usr") if random.random() > 0.3 else None, #User wont exist as no seeding for user table
...
def generate_users_table() -> list[dict]:
    #Utilized in the change logs and issues table
    #Currently generates random user id for each record
    pass
